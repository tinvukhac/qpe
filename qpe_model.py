...
class QueryPerformanceEstimatorModel(object):
    ...
    def __init__(self, state_size):
        ...
        nrows = (td.GetItem('rows') >> td.Scalar(dtype='int32') >> td.Function(td.Embedding(10, state_size, name='terminal_embed')))

        def query_op(name):
            return (td.Map([('relations', relation_declaration())]) >>
                    td.Concat() >>
        ...
        cases = td.OneOf(lambda x: x['op'], case_blocks)
        ...
        relation_label = (td.GetItem('result') >> td.InputTransform(get_best_option) >> td.OneHot(NUM_LABELS))
